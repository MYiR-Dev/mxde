...
//        connect(this, SIGNAL(LedBrightnessChanged(const QString)), m_contentwidget, SLOT(onLedBrightnessChanged(const QString)));
        connect(this, SIGNAL(sigCanRecvData(int,int, int, QString)), m_contentwidget, SLOT(onCanRecvData(int,int, int, QString)));
    }
...
void MxMainDialog::onCanRecvData(int can_fd,int can_id,int can_dlc,const QString &data)
{
    emit this->sigCanRecvData(can_fd, can_id, can_dlc, data);
}
